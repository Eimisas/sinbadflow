...
class ExecutorTest(unittest.TestCase):
    ...
    def test_should_run_simple_pipeline(self):
        pipeline = Element('ok1') >> Element('ok2')
    ...
    def test_should_run_parallel_pipeline(self):
        pipeline = Element('ok1') >> [Element('ok2'), Element('ok3')]
        self.sf_run(pipeline)
    ...
    def test_should_get_head_from_pipeline(self):
        root = Element([Element('ok1')])
        pipeline = root >> Element('ok2')
        self.sf_run(pipeline)
    ...
    def test_should_trigger_fail_all(self):
        pipeline = Element('fail') >> Element('ok', Trigger.FAIL_ALL)
        self.sf_run(pipeline)
        output = {'ok': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_ok_all(self):
        pipeline = Element('ok1') >> Element('ok2', Trigger.OK_ALL)
        self.sf_run(pipeline)
        output = {'ok1': 1, 'ok2': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_fail_prev(self):
        pipeline = Element('fail') >> Element('ok', Trigger.FAIL_PREV)
        self.sf_run(pipeline)
        output = {'ok': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_ok_prev(self):
        pipeline = Element('ok1') >> Element('ok2', Trigger.OK_PREV)
        self.sf_run(pipeline)
        output = {'ok1': 1, 'ok2': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_done_prev(self):
        pipeline = Element('fail') >> Element(
            'ok2', Trigger.OK_PREV) >> Element('ok3')
        self.sf_run(pipeline)
        output = {'ok3': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_works_parallel_notebooks(self):
        pipeline = Element('fail') >> [Element('ok1', Trigger.OK_ALL), Element(
            'ok2', Trigger.OK_ALL), Element('ok3', Trigger.FAIL_PREV)]
        self.sf_run(pipeline)
        output = {'ok3': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_trigger_works_at_parallel_notebooks_output(self):
        pipeline = [Element('fail'), Element('ok2')] >> Element('ok3', Trigger.FAIL_PREV)
        self.sf_run(pipeline)
        output = {'ok2': 1, 'ok3': 1}
        self.assertTrue(self.run_store == output,
                        f"Should get {output}, got: {self.run_store}")
    ...
    def test_should_skip_all_after_fail(self):
        pipeline = Element('fail') >> Element('ok2', Trigger.OK_PREV) >> Element(
            'ok3', Trigger.OK_ALL) >> [Element('fail', Trigger.OK_PREV), Element('ok2', Trigger.OK_PREV)]
        self.sf_run(pipeline)
        output = {}
        self.assertTrue(
    ...
    def test_should_connect_pipelines_and_run_all(self):
        pipeline = Element('ok1') >> Element('ok2')
        pipeline2 = Element('ok3') >> [Element('ok4'), Element('ok5')]
    ...
    def test_should_get_correct_statuses(self):
        pipeline = Element('fail') >> Element('ok2', Trigger.OK_PREV) >> Element(
            'ok3', Trigger.FAIL_PREV) >> [Element('fail', Trigger.OK_PREV), Element('ok2', Trigger.OK_PREV)]
        self.sf_run(pipeline)
